...
for i in range(0, len(test_lines)):
	...
	for j in range(1,9):
		...
		for k in range(0,len(splitline)-1):
			word1 = splitline[k]
			word2 = splitline[k+1]
			if ((class_word_map[j]).get((word1,word2)) == None):
				theta_sum[j] += math.log(1.0/total)
			else:
				theta_sum[j] += math.log(((class_word_map[j])[(word1,word2)]+1)/total)
