...
def back_propagation(layers,layers_output,trainY,trainX):
    ...
    layers_delta[n-1] = np.asarray((-1)* np.array(trainY-x0)*sigmoid_gradient(x0))
    # print "layers_delta[n-1]", layers_delta[n-1]
    for i in xrange(2,n+1):
        x1 = np.asarray(layers_output[n-i])
        y1 = layers_delta[n-i+1].dot(layers[n-i+1].T)
        layers_delta[n-i] = np.asarray(y1*relu_gradient(x1))
...
def train_sgd(trainX, trainY, layers, batch_size):
    ...
    while 1:
        ...
        for i in range(0, batch_size):
            layers_output[0] = relu(np.dot(X_train[i], layers[0]))
            for k in range(1,len(layers)-1):
                layers_output[k] = relu(np.dot(layers_output[k-1], layers[k]))
            layers_output[len(layers)-1] = sigmoid(np.dot(layers_output[len(layers)-2], layers[len(layers)-1]))
